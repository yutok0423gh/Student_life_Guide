...
- 首页场景、八大分类、搜索、文章详情、紧急入口、收藏、历史、清单、地区、学校资料与“我的”
- 搜索严格限定为 `title`、`summary`、`keywords`、`aliases`，支持中文近似表达
- 地区、学校、校区、专业、收藏、历史与清单状态保存在浏览器本地；无登录、后端、广告或第三方追踪
- 内置教育部截至 2026-06-17 的 2952 所普通高校名单，支持正式名称、常用简称和地区优先搜索；找不到时可手动填写
- Service Worker 缓存界面与随包内容，首次加载后可断网阅读
- 构建时从 Android `assets/content/guides/` 机械同步内容，不维护第二份手写正文
- 30 项 Node 测试与内容/学校目录/远程依赖检查；已完成桌面、390 px 手机视口和断开本地服务器后的浏览器验证
